...
export default function UebungenPage() {
  ...
  return (
    <PageShell
      title="Übungen"
      subtitle={`${records.length} Übungen im System`}
      action={
    ...
    >
      ...
      <div className="rounded-lg border bg-card overflow-hidden">
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead>Übungsname</TableHead>
              <TableHead>Muskelgruppe</TableHead>
              <TableHead>Equipment</TableHead>
              <TableHead>Schwierigkeitsgrad</TableHead>
          ...
          <TableBody>
            {filtered.map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors">
                ...
                <TableCell><Badge variant="secondary">{record.fields.muskelgruppe ?? '—'}</Badge></TableCell>
                <TableCell><Badge variant="secondary">{record.fields.equipment ?? '—'}</Badge></TableCell>
                <TableCell><Badge variant="secondary">{record.fields.schwierigkeitsgrad ?? '—'}</Badge></TableCell>
                <TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={5} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Übungen. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
